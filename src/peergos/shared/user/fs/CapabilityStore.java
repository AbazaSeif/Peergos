...
public class CapabilityStore {
    public static final int READ_CAPABILITY_SIZE = 159; // fp.toCbor().toByteArray() DOESN'T INCLUDE .secret
    public static final int EDIT_CAPABILITY_SIZE = 159;
    ...
    public static final String CAPABILITY_READ = ".r.";
    public static final String CAPABILITY_EDIT = ".w.";
    public static final String READ_ONLY_SHARING_FILE_PREFIX = "sharing" + CAPABILITY_READ;
    ...
    private static final Comparator<FileWrapper> readOnlyCapabilitiesInIndexOrder =
            Comparator.comparingInt(f -> filenameToIndex(f.getName(), READ_ONLY_SHARING_FILE_PREFIX));
                    ...
                    return getSharingCacheFile(friendName, homeDirSupplier, network, random, capabilityType).thenCompose(optCachedFile -> {
                        ...
                        if(! optCachedFile.isPresent() ) {
                            CompletableFuture<List<CapabilityWithPath>> allFiles = Futures.reduceAll(sharingFiles,
                                    ...
                                    (res, sharingFile) -> readSharingFile(friendSharedDir.getName(), capabilitySize, sharingFile, network, random)
    ...
    public static CompletableFuture<Long> getReadOnlyCapabilityCount(FileWrapper friendSharedDir,
                                                                     NetworkAccess network) {
        return getCapabilityCount(READ_ONLY_SHARING_FILE_PREFIX, friendSharedDir, network);
    }
    ...
    public static CompletableFuture<List<CapabilityWithPath>> readSharingFile(String ownerName,
                                                                              ...
                                                                              NetworkAccess network,
                                                                              SafeRandom random) {
        return readSharingFile(0, capabilitySize, ownerName, file, network, random);
    }
    ...
    public static CompletableFuture<CapabilitiesFromUser> saveRetrievedCapabilityCache(long recordsRead,
        ...
        return getCapabilityCacheDir(homeDirSupplier, network, random)
                .thenCompose(cacheDir -> cacheDir.uploadFile(friend + suffix, dataReader, true, (long) data.length,
